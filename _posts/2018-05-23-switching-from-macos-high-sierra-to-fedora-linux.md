...
I'm writing this mainly as a high-level migration guide for those jumping into Linux - specifically, from macOS to Fedora. It also contains some strategies and principals for maintaining cross-platform configurations. It will not cover much of the nitty-gritty configuration details, as those are in my [dotfiles [repo](https://github.com/sh78/dotfiles/),](http://repo]%28https//github.com/sh78/dotfiles/),) which just might get a nice README some day.

Note that everything here was done on a Early 2015 Retina MacBook Pro running Fedora 28 latest as of 2018-05-24. Also note that this is a likely a **multi-day** project, unless you have an unsafe amount of caffeine.

# Why?

Free and open source is good; yes, I know, Fedora isn't technically 100% "free" [according to Richard [Stallman/FSF](https://www.gnu.org/distros/common-distros.en.html#Fedora)](http://stallman/FSF](https://www.gnu.org/distros/common-distros.en.html#Fedora)).
...
So, without further adieu, here is the play-by-play migration of approximately 10 years' worth of workflow, built upon the Apple eco-system, to a modern Linux environment - in chronological order.

# Hardware

## The Host Machine

Everything you see here (and more :wink) was done using an early 2015 Retina MacBook Pro 13". I'm quite happy with how well Fedora worked out of the box. Immediately on boot the hidpi display looked stellar, the WiFi worked (suck it Arch users), and the damn *volume/brightness keys worked*. It really is 2018.

## Portability, Speed, and Redundancy With A Bootable External Drive

use of bootable ssd with partition for each os

Another nifty angle to the portable drive set up is that I left a 100GB partition on there to serve as my bootable macOS. Using something like [Carbon Copy Cloner]() it's easy to maintain a [bootable clone of your mac](). Better yet, I run my mac from the external drive and clone it to the internal as backup, so I'm always ready to travel light. I used to take my personal laptop to work, in case I needed to work on something personal before or after work. Now I just bring the drive and plug it in to a work machine or a public computer if I really need to get something done that's not easy on a phone or someone else's browser.

Oh, and the portable SSD over USB3 or thunderbolt is wicked fast.

Reminisce about running splice server in Nepal over usb2 on a 2010 mbp i3
...
![The Fedora hot dog inspired confidence from the start](https://pbs.twimg.com/media/DdxtLScVAAET6Lc.jpg)
...
So, obviously, I'm definitely not using this.

For now, though, the built in terminal and a good browser will suffice.
...
![](https://media.giphy.com/media/10lBhcF4eTJZWU/giphy.gif)
...
• [1Password]() - password manager. More on this later.
• [Pocket]() - read it later doohickey
• [Mercury Reader]() - read it now doohickey. I feel like this should be built into Chrome by now, but I digress.
*[TypeFu]() - and extremely excellent typing trainer that adjust to your skill level, tracks your progress with graphs, and even lets you practice against code and upload your own templates. It's a one-time $5 purchase and works anywhere that Chrome does. 
• [Pushbullet]() - monitor/act on phone notifications, sync SMS, and unified copy/paste. Suck it Apple.
• [Don't Fuck With Paste]() - just what it says. Thwarts nefarious developers that assume you are not responsible enough to paste in your password/account number/what have you.
• [Dev Tools]() - this is of course included by default, but just a hat tip for how excellent Chrome Dev Tools is.

And while we're at it, web apps:

* Google's things - Gmail, Drive, Calendar, and Keep are my core utils. Keep is not well promoted but it's a fantastic unified note space with support for #tagging and images. Also Hangouts is great for making calls, but I'm not sure if this works for everyone or just Project Fi subscribers.
* [ExplainShell](https://explainshell.com/) - breaks down shell commands into their components and explains them
* [RegExr](https://regexr.com/) - the Samuel L Jackson of browser-based Regex prototyping.
* [Pixlr](https://pixlr.com/editor/) - photo manipulation. I don't have very advanced photo needs any more, and web optimization happens in the terminal.
...
[Getting the local .cvimrc loaded was a bit on tricky on macOS](https://github.com/1995eaton/chromium-vim/issues/312#issuecomment-156189327), I recall. ![Also be sure to "allow access to file URLs" from the chrome://extensions/settings](https://i.imgur.com/jnH8YmH.png).
...
Close call here - I totally forgot that 1Password wasn't on Linux. I was about to call it all off, as 1Password has become an invaluable tool in my arsenal. Well, actually, I was going to suck it up and just use the web interface, which is nice but not native. Before saddling up, I did a quick web search and BAM - [1Password X](https://blog.agilebits.com/2017/11/13/1password-x-a-look-at-the-future-of-1password-in-the-browser/) 🎉

There's also a [1Password CLI](https://app-updates.agilebits.com/product_history/CLI) that I'd like to test drive, but not today.
...
Thinking back to the time that I ended up being in a coma for one month, I wish we had been on the 1Password family plan so that my bank information and logins were stored in a shared vault accessible to my parents and my wife. I still remember how difficult it was to explain what my computer's password was, and worse yet the unlock pattern for my phone, while unable to speak due to being intubated and too paralyzed to write. 
...
We could also just test for existence of the commands, like `test (which pbcopy)...`, but I like this method becuase it reads well and I plan on setting more environment vars later on.
...
I've been happily using Google Drive to sync my configs and working files for quite some time now. Well, there's no official Google Drive client for Linux. I read [some clickbaits about Google Drive on Linux](https://www.howtogeek.com/196635/an-official-google-drive-for-linux-is-here-sort-of-maybe-this-is-all-well-ever-get/) and tried some of the most viable seeming options.

[Drive clients](https://beebom.com/unofficial-google-drive-linux-clients/)

### GNOME

Unfortunately the GNOME Drive integration that ships with Fedora doesn't actually sync. I tried clicking the mount just to see, and 10 minutes later I still couldn't even get a file listing. Yuck.
...
![]([https://media.giphy.com/media/e9i8NXW1GSqAg/giphy.gif](https://media.giphy.com/media/e9i8NXW1GSqAg/giphy.gif)
...
Et voilà! It returned "9053 files queued".  We may just use truncated output of  `insync get_sync_progress` in a future i3 status bar... Stay tuned.

So, at this point there's still nothing in my ~/Drive dir that I chose... In the docs, they show additional steps where you pick which folders to sync and can choose some settings. I didn't see that; when I chose the sync location, the InSync window just disappeared.
...
Once files are synced, we're ready to make this house a home.
...
I use [neovim](https://github.com/neovim/neovim), but also like to keep vim 8 around just in case.
...
There was surprisingly little to debug here. On launch my auto-magical Vundle script installed all my plugins, and there were no warnings/incompatibilities with other stuff in my [vimrc](). The main issue was a whacked out colorscheme (from using Solarized palette in iTerm) and weird characters in the [airline]() status line.

![My accidental rave colorscheme in vim](https://i.imgur.com/cAYDEuA.png)

Now let's pick a new terminal, and figure out about installing fonts to tidy up that statusline..

## Choosing A Terminal Emulator

There are quite literally a shit-ton of terminal emulators for Linux out there. Coming from [iTerm](https://www.iterm2.com/), there's some pretty big shoes to fill here. I was about to go with [urxvt](https://www.youtube.com/watch?reload=9&v=eaBf_yFHps8), but then remembered that I hadn't checked on [Hyper](https://hyper.is/) in awhile.

Hyper is continuing to grow, and has some [awesome extensions](https://github.com/bnb/awesome-hyper) going for it that cover much of what I use in iTerm. The big winner here is that Hyper's configuration is plain ol' .js object, which is perfect for cross-platform syncing.
...
[Here's my .hyper.js config](https://github.com/sh78/dotfiles/blob/master/.hyper.js).

### Bonus: Just For Kicks

[Cool Retro Terminal]() is pretty neat. I keep it around. Great for when clients come to the office and we need to look professional.
...
![And Behold!](https://i.imgur.com/rSQA4gc.jpg)
![Hi](https://i.imgur.com/uFn4rGf.jpg)
...
At this point I'm **still** waiting for my Google Drive sync to complete; it seems a purge is in order.... Just for a sneak preview, I copied my .tmux.conf in and gave it a whirl.
...
So how to keep this working on macOS but not on Linux? `if-shell` works nicely here. We can split out OS-specific comands into separate files and `source` then:
...
I couldn't find this exact error in a web search but noticed that my ranger version was several releases behind what's listed on the [official site](https://ranger.github.io/download.html).Time to build from source.
...
With ranger master, `zh` works as expected, divulging the whereabouts of hidden files. Hooray for hyposthesis!
...
When I heard on the internet that Fedora and vagrant have had a checkered past, I was concerned. Luckily some kind and brave soul at if not true then false published a great [step by step](https://www.if-not-true-then-false.com/2010/install-virtualbox-with-yum-on-fedora-centos-red-hat-rhel/). Here's a summary.
...
![Pics or it didn't happen O.o](https://i.imgur.com/LqeuS1Y.png)

There's also [another wonderful guide from if not true then false](https://www.if-not-true-then-false.com/2010/install-virtualbox-guest-additions-on-fedora-centos-red-hat-rhel/) on how to get Guest Additions set up.
...
I have some automagic iTerm color scheme change logic based on time of day, but we're a long way from Kansas now, so let's wrap that conditional in a conditional, yo.
...
Here are other programs I like or find useful for development and general workflow:

[dotfiles/installers array]()
...
I like setting unique hostnames for my machines. I pick a quality about the machine, then translate that word to Turkish.

sudo hostnamectl set-hostname sapka

"şapka" is Turkish for "hat" (RedHat). My first retina MacBook Pro was named "göz" for "eye". My MacBook "Air" was named "hava" (rough translation). Etcetera, etcetera.

## Automation Is King

On macOS, there is `defaults write`, and `defaults export`/`import`. So this is linux, can we automate Fedor's preferences in such a fasion?
...
[Here's a guide on screenshots is Fedora](https://fedoramagazine.org/take-screenshots-on-fedora/). I made a custom keyboard shortcut of Command+Shift+4 because I don't have a Print Screen key and it's familiar.

## How To Special Characters/Symbols And Insert Emoji

I use Command+Control+t on the Mac to insert Emoji and whatnot. Fedora has a similar menu but you have to [enable it [first](https://fedoramagazine.org/using-favorite-emoji-fedora-25/)](http://first]%28https//fedoramagazine.org/using-favorite-emoji-fedora-25/)).
